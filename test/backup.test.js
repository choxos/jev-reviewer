...
test("backup and restore: projects, questions, studies, answers and files come back as new projects", async () => {
  ...
  assert.equal(table, 'study,authors,year,title,journal,doi,pmid,excluded,study_note,checked,age,age quotes,phq9,phq9 quotes\r\nJohnson 2026,"Johnson, E",2026,A trial,PLoS Med,10.1/x,1,Wrong population,Asked the authors for SDs,1 of 2,18 to 65,"""Adults"" (trial.pdf, p. 1)","iCBT: 8.1 (4.2), n = 120",Reported\r\n');
  // excluded studies have no place in the outcome data (this one is excluded), so only the header
  assert.equal(await openZip(archive).text("1 Depression review outcome data.csv"), "study,authors,year,doi,outcome,outcome_question,kind,arm,n,mean,sd,events,checked\r\n");
